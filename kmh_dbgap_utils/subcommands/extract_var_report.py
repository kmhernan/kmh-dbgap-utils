...
from kmh_dbgap_utils.parsers import VariableReportParser


class ExtractVariableReport(Subcommand):
    @classmethod
    ...
    def __get_description__(cls):
        ...
        """
        return "Download variable reports and make TSV."

    @classmethod
    def main(cls, options): 
        logger = Logger.get_logger(cls.__tool_name__())
        logger.info("Download dbGap variable reports.")
        ...
        ofile = os.path.join(options.outdir,
                             '{fullid}.variable_report.tsv'.format(**phs))
        logger.info("Extracting outputs to {0}".format(ofile))
        ohead = ['study_phs', 'study_name', 
                 'xml_file'] + VariableReportParser.columns()
        ...
        with open(ofile, 'wt') as o:
            o.write('\t'.join(ohead) + '\n')
            for xml_file, dat in dbgap.iter_study_xmls(phs, 'var_report.xml',
                                                       options.outdir):
                parser = VariableReportParser(dat)
                for record in parser:
                    if not record:
                        continue
                    row = [phs['fullid'], study_name, xml_file] + [
                           record[i] for i in VariableReportParser.columns()]
                    o.write('\t'.join(row) + '\n')
